...
function getPolynom(...coefficients) {
  return function polynom(x) {
    ...
    for (let i = 0; i < coefficients.length; i += 1) {
      ...
      polynomResult += x ** power * coefficients[i];
    }
...
function logger(func, logFunc) {
  return function loggerFn(...args) {
    const argumentsFn = args.map((arg) => JSON.stringify(arg)).join(',');
    const functionName = func.name;
    logFunc(`${functionName}(${argumentsFn}) starts`);
    const log = func(...args);
    logFunc(`${functionName}(${argumentsFn}) ends`);
    return log;
  };
}
...
function partialUsingArguments(fn, ...args) {
  return function apply(...moreArgs) {
